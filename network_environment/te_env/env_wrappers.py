...
from mappo_fcn.utils.util import tile_images
...
class ShareVecEnv(ABC):
    ...
    def __init__(self, num_envs, global_space, observation_space, action_space):
...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            state, ob, reward, done, info = env.step(data)
            ...
            remote.send((state, ob, reward, done, info))
        elif cmd == 'reset':
            state, ob = env.reset()
            remote.send((state, ob))
        elif cmd == 'render':
            if data == "rgb_array":
            ...
            # print(marl_env.observation_space)
            remote.send((env.global_space, env.observation_space, env.action_space))
        else:
            raise NotImplementedError
...
class SubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.remotes[0].send(('get_spaces', None))
        global_space, observation_space, action_space = self.remotes[0].recv()
        ShareVecEnv.__init__(self, len(env_fns), global_space, observation_space, action_space)
    ...
    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        state, obs = map(np.array, zip(*results))
        return state, obs
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        state, obs, action_mask, rews, dones, infos = map(np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    state[i], obs[i], action_mask[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    state[i], obs[i], action_mask[i] = self.envs[i].reset()

        self.actions = None
        return state, obs, action_mask, rews, dones, infos
