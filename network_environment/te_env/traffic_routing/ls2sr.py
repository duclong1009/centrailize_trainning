...
class LS2SRSolver(Solver):

    def __init__(self, graph, timeout, verbose, args=None):
        super(LS2SRSolver, self).__init__(graph, timeout, verbose)
    ...
    def initialize_flow2link(self):
        ...
        for i, j in itertools.product(range(self.num_node), range(self.num_node)):
            flow2link[i, j] = list_paths[i * self.num_node + j]
    ...
    def compute_path(self):
        path = os.path.join(self.args.data_folder, f'topo/ls{self.args.n_sr}sr_segment_{self.args.dataset}.pkl')
    ...
    def get_solution_bound(self, flow2link):
        ub = np.empty([self.num_node, self.num_node], dtype=int)
        for i, j in itertools.product(range(self.num_node), range(self.num_node)):
    ...
    def initialize_solution(self):
        return np.zeros(shape=(self.num_node, self.num_node), dtype=int)
    ...
    def select_flow(self, tm):
        ...
        try:
            ...
            self.link_selection_prob[self.link_selection_prob <= 0] = 0.0
            idx = np.random.choice(self.indices_edge, size=1, p=self.link_selection_prob)
    ...
    def solve(self, tm=None, solution=None, eps=1e-12):
        ...
        u = self.evaluate(tm=tm, solution=solution)
        theta = u
