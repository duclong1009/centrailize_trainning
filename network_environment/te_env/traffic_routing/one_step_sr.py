...
class OneStepSRSolver(Solver):

    def __init__(self, graph, segment, timeout, verbose):
        super(OneStepSRSolver, self).__init__(graph, timeout, verbose)
        """
    ...
    def solve(self, tm, solution=None, eps=1e-12):
        ...
        # end_time = time.time()
        # print('Time for creating problem:', end_time - start_time)
        # start_time = time.time()
        problem.solve(solver=self.solver)
        # end_time = time.time()
        # print('Time for solving problem:', end_time - start_time)
