...
class DatanetAPI:
    ...
    def set_files_to_process(self, tuple_files_lst):
        ...
        if not type(tuple_files_lst) is list:
            raise DatanetException(
                "ERROR: The argument of set_files_to_process should be a list of tuples -> [(root_dir,file),...]")
        for tuple in tuple_files_lst:
            ...
            if (not tuple in self._all_tuple_files):
                raise DatanetException(
                    "ERROR: Selected tupla not belong to the list of tuples returned by get_available_files()")
    ...
    def _readRoutingFile(self, routing_file, netSize):
        ...
        ----------
        routing_file : str
            File where the traffic_routing information is located.
        netSize : int
    ...
    def _create_routing_matrix_from_dst_routing_file(self, G, routing_file):
        ...
        routing_file : str
            File where the information about traffic_routing is located. The file is a
            destination traffic_routing file.
        Returns
        -------
    ...
    def _create_routing_matrix_from_src_routing_dir(self, G, src_routing_dir):
        ...
        G : graph
            Graph representing the network.
        src_routing_dir : str
            Directory where we found the traffic_routing filesFile. One for each src node.
        Returns
        -------
        MatrixPath : NxN Matrix
            Matrix where each cell [i,j] contains the path to go from node
    ...
    def _create_routing_matrix(self, G, routing_file):
        ...
        G : graph
            Graph representing the network.
        routing_file : str
            File where the information about traffic_routing is located.
        Returns
        -------
        MatrixPath : NxN Matrix
            Matrix where each cell [i,j] contains the path to go from node
            ...
            try:
                ...
                while (True):
                    ...
                    if len(self.intensity_values) != 0:
                        ...
                        if (specific_intensity < self.intensity_values[0]) or (
                                specific_intensity > self.intensity_values[1]):
                            continue
                    ...
                    # XXX We considerer that all graphs using the same traffic_routing file have the same topology
                    ...
                    self._process_flow_results_traffic_line(s._results_line, s._traffic_line, s._flowresults_line,
                                                            s._status_line, s)
    ...
    def _process_flow_results_traffic_line(self, rline, tline, fline, sline, s):
        ...
        for i in range(0, len(r), int(math.sqrt(len(r)))):
            ...
            for j in range(i, i + int(math.sqrt(len(r)))):
                ...
                dict_result_agg = {'PktsDrop': aux_agg[2], "AvgDelay": aux_agg[3], "AvgLnDelay": aux_agg[4],
                ...
                for flow in aux_result_flows:
                    ...
                    dict_result_tmp = {'PktsDrop': tmp_result_flow[2], "AvgDelay": tmp_result_flow[3],
                                       "AvgLnDelay": tmp_result_flow[4], "p10": tmp_result_flow[5],
                                       "p20": tmp_result_flow[6], "p50": tmp_result_flow[7], "p80": tmp_result_flow[8],
                                       "p90": tmp_result_flow[9], "Jitter": tmp_result_flow[10]}
